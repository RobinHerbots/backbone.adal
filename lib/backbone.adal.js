...
Backbone.AdalRouter = Backbone.AdalRouter.extend({
    ...
    _callback: function (errorDesc, token, error) {
        if (errorDesc) {
            this.trigger('errorAuthenticate', [errorDesc]);
        } else {
...
Backbone.sync = function (method, model, options) {
    const that = this,
        ...
        resource = authContext.getResourceForEndpoint(model.url);

    if (resource !== null) {
        authContext.acquireToken(resource, function (error, token) {
            if (error || !token) {
                that.trigger("errorAuthenticate", [error]);
                return;
            }
            ...
            return originSync.call(that, method, model, options);
        });
    }
}